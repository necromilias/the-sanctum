...
export default function Dashboard() {
  const { status, horde, lastCheck } = useStatus();
  ...
  const hordeSummary = !horde
    ...
    : `${horde.online ? 'worker online' : 'worker offline'} · ${horde.served.toLocaleString()} served`;

  return (
    <main className="relative max-w-5xl mx-auto px-4 sm:px-6 md:px-8 py-6 md:py-8 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-baseline justify-between gap-2 mb-6">
        <div>
          <p className="font-mono text-[11px] tracking-widest text-cyan-500/70 uppercase mb-1.5">Personal systems overview</p>
          <h1 className="text-2xl font-semibold text-ghost-50">Sanctum status</h1>
        </div>
        <span className="font-mono text-xs text-ghost-400">
          2 systems · {services.length} featured services · {lastCheck ? `checked ${lastCheck.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}` : 'checking…'}
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-3">
        <Link to="/systems" className="block glass-panel p-5 hover:border-cyan-500/40 transition-all">
          <div className="flex items-center justify-between mb-2.5">
            <span className="font-mono text-sm text-ghost-100">the-forge</span>
            <span className="font-mono text-[11px] text-ghost-500">WORKSTATION</span>
          </div>
          <div className="font-mono text-xs text-ghost-400 flex flex-col gap-0.5">
            <span>cachyos · workstation</span>
            <span className="text-cyan-400">AI Horde · WhisperX · {hordeSummary}</span>
          </div>
        </Link>
        <Link to="/systems" className="block glass-panel p-5 hover:border-cyan-500/40 transition-all">
          <div className="flex items-center justify-between mb-2.5">
            <span className="font-mono text-sm text-ghost-100">the-trove</span>
            <span className="font-mono text-[11px] text-ghost-500">SERVER</span>
          </div>
          <div className="font-mono text-xs text-ghost-400 flex flex-col gap-0.5">
            <span>ubuntu server · docker host</span>
            <span className="text-cyan-400">{services.length} featured services</span>
          </div>
        </Link>
      </div>
        ...
        {services.map((svc) => {
          ...
          return (
            <div key={svc._key} className="flex items-center gap-4 px-5 py-3 border-b border-ghost-700 last:border-b-0 hover:bg-cyan-500/5 transition-colors">
              ...
              <span className="hidden sm:inline font-mono text-[11px] text-ghost-500">{statusLabel(s)}</span>
            </div>
          );
        })}
      </div>
